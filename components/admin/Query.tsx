...
import {
    ...
    TrendingUp,
    Users,
    AlertCircle, ArrowRight, ArrowUp, MoveUpRight,
} from "lucide-react"
...
export default function Query({ data }: { data: Query[] }) {
    const [queries, setQueries] = useState<Query[]>(data)
    ...
    const filteredQueries = queries.filter((query) => {
        ...
        if (activeFilter === "solved") return query.isSolved
        if (activeFilter === "unsolved") return !query.isSolved
        return true
    })

    const solvedCount = queries.filter((q) => q.isSolved).length
    const unsolvedCount = queries.filter((q) => !q.isSolved).length
    const solvedPercentage = queries.length > 0 ? Math.round((solvedCount / queries.length) * 100) : 0
    ...
    return (
        <section className="min-h-screen bg-black relative overflow-hidden">
            ...
            {/* Content */}
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 pt-20 relative z-10">
                {/* Header Section */}
              ...
              Queries Dashboard
                    ...
                    <p className="text-xl md:text-2xl text-gray-300 mb-12 max-w-4xl mx-auto leading-relaxed font-light">
                        Manage and track customer inquiries with enterprise-scale efficiency. Transform customer support into a
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-12">
                    <Card className="bg-gradient-to-br from-purple-950/40 to-purple-900/20 backdrop-blur-sm border border-purple-500/30 hover:border-purple-400/50 transition-all duration-300">
                        <CardContent className="p-6">
                            <div className="flex items-center justify-between">
                                <div>
                                    <p className="text-purple-300 text-sm font-medium">Total Queries</p>
                    ...
                    <Card className="bg-gradient-to-br from-green-950/40 to-green-900/20 backdrop-blur-sm border border-green-500/30 hover:border-green-400/50 transition-all duration-300">
                        <CardContent className="p-6">
                            <div className="flex items-center justify-between">
                                <div>
                                    <p className="text-green-300 text-sm font-medium">Solved</p>
                        ...
                        <CardContent className="p-6">
                            <div className="flex items-center justify-between">
                                <div>
                                    ...
                                    <p className="text-3xl font-bold text-white">{unsolvedCount}</p>
                                </div>
                ...
                {filteredQueries.length === 0 ? (
                    <div className="text-center py-20">
                        ...
                        <h3 className="text-2xl font-bold text-white mb-4">
                            No {activeFilter !== "all" ? activeFilter : ""} Queries Found
                        </h3>
                        <p className="text-gray-300 text-lg">
                            {activeFilter === "all"
                                ? "There are no customer queries to display at the moment."
                                : activeFilter === "solved"
                                    ? "There are no solved queries to display at the moment."
                                    : "There are no pending queries to display at the moment."}
                        </p>
                    </div>
                ) : (
                    ...
                    <div className="relative ">
                            ...
                            <h2 className="text-2xl font-bold text-white">
                                {activeFilter === "all"
                                    ...
                                    : activeFilter === "solved"
                                        ? "Solved Queries"
                                        : "Pending Queries"}
                                <span className="ml-2 text-sm text-gray-400">({filteredQueries.length})</span>
                            </h2>
                        ...
                        <div
                            ref={carouselRef}
                            className="flex overflow-x-auto gap-6 p-4  snap-x snap-mandatory hide-scrollbar"
                            style={{ scrollbarWidth: "none", msOverflowStyle: "none" }}
                        >
                            {filteredQueries.map((query, index) => (
                                <Card
                                    key={query.id}
                                    className={`group flex-shrink-0 snap-start w-[350px] bg-gradient-to-b from-purple-950/30 to-black/50 backdrop-blur-sm border border-purple-500/20 hover:border-purple-400/40 transition-all duration-500 hover:transform hover:scale-[1.02] shadow-lg shadow-purple-500/10 hover:shadow-purple-500/20 ${
                                        index % 4 === 0
                                ...
                                >
                                    <CardHeader className="pb-4">
                                        <div className="flex items-start justify-between">
                                            ...
                                            <div className="flex items-center gap-2">
                                                <Button
                                                    onClick={() => router.push(`/admin/${query.id}`)}
                                                    variant="outline"
                                                    size="icon"
                                    ...
                                    <CardContent className="space-y-4">
                                        <div className="flex items-center space-x-3 text-gray-300 group-hover:text-gray-200 transition-colors duration-300">
                                            ...
                                            <span className="truncate font-medium text-sm">{query.email}</span>
                                        </div>
                                        ...
                                        {query.company && (
                                            <div className="flex items-center space-x-3 text-gray-300 group-hover:text-gray-200 transition-colors duration-300">
                                                ...
                                                <span className="truncate font-medium text-sm">{query.company}</span>
                                            </div>
                                        )}
                                        ...
                                        <div className="flex items-start space-x-3 text-gray-300 group-hover:text-gray-200 transition-colors duration-300">
                                            ...
                                            <p className="line-clamp-3 leading-relaxed text-sm">{query.message}</p>
                                        </div>
